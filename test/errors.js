...
test('one file and no banner', async t => {
	const {stderr, code} = await t.throwsAsync(cli(['test/fixtures/a.css']));
...
test('no file and a banner', async t => {
	const {stderr, code} = await t.throwsAsync(cli(['-b']));
...
test('non-existing files', async t => {
	const {stderr, code} = await t.throwsAsync(cli(['test/fixtures/non-existing.css', 'test/fixtures/a.css']));
...
test('Output as a directory', async t => {
	...
	const {stderr, code} = await t.throwsAsync(cli(['test/fixtures/a.css', 'test/fixtures/b.css', '-o', output]));
