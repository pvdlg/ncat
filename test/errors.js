...
test('one file and no banner', async t => {
  const {stderr, code} = await t.throws(cli(['test/fixtures/a.css']));
...
test('no file and a banner', async t => {
  const {stderr, code} = await t.throws(cli(['-b']));
...
test('non-existing files', async t => {
  const {stderr, code} = await t.throws(cli(['test/fixtures/non-existing.css', 'test/fixtures/a.css']));
